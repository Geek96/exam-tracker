...
**每次新会话开始时，在执行任何任务之前，必须按顺序完成以下步骤：**

1. **读 `STATUS.md`** — 了解当前功能状态、已知问题、环境变量配置、最近提交
2. **读 `ROADMAP.md`** — 了解项目愿景和开发优先级，避免做出与路线图冲突的设计决策
3. **确认 `course.js` 当前版本号**（见本文件第 2 节），确保修改后正确递增

> ❌ 跳过上述步骤直接动手 = 违反协议。你可能会重复已修复的 bug、实现已废弃的方案、或与既定路线图冲突。
...
当前版本：`v=31`，下一批次目标：`v=32`（Task1）→ `v=33`（Task3）。
...
- 在 `STATUS.md` 的当前任务中追踪所完成任务(在完成任务后进行询问:接受, 拒绝, 修改三个选项) 1. 选择接受则将文件当前任务栏中删去任务. 并在superpower/plans的相关 md文件名前添加‘已完成’ 2. 拒绝则保留任务 3. 修改则让用户提出新的要求以修改当前任务目标
