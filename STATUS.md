...
| AI 教材片段检索 | ✅ 正常 | Markdown 教材自动分块；支持索引版本自动重建、裸编号习题识别、目录降权、章节/题号优先召回，检索为空时按问题定位相关摘录并引导选择可检索范围 |
...
```
本次工作  fix: target material fallback excerpts
333c423  docs: require visible version sync in commits
81ffe84  feat: improve material rag exercise retrieval
2528a84  fix: guide users when material retrieval misses
```
...
| P10 | RAG 结构化索引与习题级召回 | 🟢 本地完成，待验收 | v=42 | `material-rag.js` 索引版本自动重建、裸编号习题识别、目录降权、目标章节摘录 |
