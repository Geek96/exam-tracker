...
| AI 教材片段检索 | ✅ 正常 | Markdown 教材自动分块；支持裸编号习题识别、目录降权、章节/题号优先召回，检索为空时回退注入已选资料摘录并引导选择可检索范围 |
...
```
本次工作  feat: improve material rag exercise retrieval
...
| P10 | RAG 结构化索引与习题级召回 | 🟢 本地完成，待验收 | v=41 | `material-rag.js` 裸编号习题识别、目录降权、doc/block/page metadata |
